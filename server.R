...
my_server <- function(input, output) {
  ...
  output$conclusion_to_grph <- renderText({
    ...
    min_adults <- filter(filtered_3(), adults_arrests == min(adults_arrests)) %>% 
      select(year)
    max_juvenile <- filter(filtered_3(), juvenile_arrests == max(juvenile_arrests)) %>% 
      select(year)
    min_juvenile <- filter(filtered_3(), juvenile_arrests == min(juvenile_arrests)) %>% 
      select(year)
    max_adults <- filter(filtered_3(), adults_arrests == max(adults_arrests)) %>% 
      select(year)
